...
const updatePlayer = function (playerId, playerName, avatarUrl, status) {
...
const addPlayer = function (id, name, avatarUrl, status, isOtherPlayer = true) {
  const player = `
          <div class="playerBox ${status}" data-id="${id}">
            <div class="avatarBox" style="background-image: url(${avatarUrl}), url('../assests/user.png')">
            </div>
...
const setAvatar = function () {
  avatarImg = getNewAvatarPanda();
  document.getElementById(
    "newUserAvatarBox"
  ).style.backgroundImage = `url(${avatarImg}), url(../assests/user.png)`;
};
...
const addPlayerToLobby = function (
  ...
  addPlayer(id, name, avatarUrl, status, isOtherPlayer);
...
addNewPlayer.addEventListener("click", async function () {
  ...
  if (!isEdit) {
    ...
    addPlayer(selfId, nameArr, avatarImg, "online", false);
    player.name = nameArr;
    player.avatarSrc = avatarImg;
    ...
    updatePlayer(selfId, nameArr, avatarImg, "online");
...
async function generateLobbyCodePanda() {
  // const pokeSeed = Math.floor(Math.random() * 1025) + 1;
  // const pokeNameResponse = await fetch(
  //   "https://pokeapi.co/api/v2/pokemon-form/" + pokeSeed
  // );
  // const pokeNameData = await pokeNameResponse.json();
  // const pokeName = pokeNameData.pokemon.name;
  // const roomNumber = Math.floor(Math.random() * 9999);
  // const zeroPaddedRoomNumber = String(roomNumber).padStart(4, "0");
  // return `poke${pokeName}${zeroPaddedRoomNumber}`;
